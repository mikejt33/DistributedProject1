...
colnames(edits_ip) <- c("Article","Date","User1","Country","Category","Comment")

edits_ip$User2 <- NA
edits_ip <- edits_ip%>% group_by(Country) %>% tally()
#add country code column 
edits_ip$CountryCode <- countrycode(edits_ip$Country,'country.name','genc3c')
